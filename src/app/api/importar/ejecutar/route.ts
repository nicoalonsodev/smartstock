...
export async function POST(request: Request) {
  ...
  const { data: categorias } = await session.supabase
    .from('categoria')
    .select('id, nombre')
    .eq('activa', true);
    ...
    try {
      ...
      if (fila.categoria) {
        ...
        if (categoriasMap.has(catNombre)) {
          ...
          const { data: nuevaCat } = await session.supabase
            .from('categoria')
      ...
      if (productoExistente) {
        ...
        if (fila.stock_actual != null && fila.stock_actual !== productoExistente.stock_actual) {
          await session.supabase.rpc('registrar_movimiento', {
        ...
        const { data: nuevoProducto, error: insertErr } = await session.supabase
          .from('producto')
          .insert({
            tenant_id: tenantId,
            codigo,
  ...
  await session.supabase.from('importacion_log').insert({
    tenant_id: tenantId,
    proveedor_id: proveedor_id,
    archivo_nombre,
    origen,
